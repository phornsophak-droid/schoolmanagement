    ...
    <div className="min-h-screen bg-[#F4F7FA] text-slate-800 font-sans flex selection:bg-blue-105 selection:text-blue-900 overflow-hidden w-full print:h-auto print:overflow-visible print:bg-white">
      ...
      <aside className={`w-64 md:w-72 bg-[#1E293B] text-white flex-col shrink-0 border-r border-slate-800/80 print:hidden h-full ${IS_PHONE ? 'hidden' : 'hidden md:flex'}`}>
        ...
        <nav className="flex-1 p-4 space-y-2 mt-4 overflow-y-auto">
          ...
          >
            <div className="flex items-center gap-3">
              <BookOpen size={16} className={activeView === 'handbook' ? 'text-blue-400' : 'text-slate-400'} />
              <span>សៀវភៅសិក្ខាគារិក</span>
            </div>
          </button>
          ...
          >
            <div className="flex items-center gap-3">
              <Library size={16} className={activeView === 'library' ? 'text-blue-400' : 'text-slate-400'} />
              <span>គ្រប់គ្រងបណ្ណាល័យ</span>
            </div>
          </button>
              ...
              <>
                ...
                {activeView === 'handbook' && (
                  <motion.div
                    ...
                    transition={{ duration: 0.15 }}
                  >
                    <Handbook students={students} grades={grades} onSaveStudents={handleSaveStudents} onClose={() => setActiveView('dashboard')} />
                  </motion.div>
                )}
