...
export default function Gradebook({
  ...
  const registeredStudentsInFormGrade = useMemo(() => {
    ...
    // Show every registered student in this grade so the monthly entry list always
    // equals the full class roster. Students who already have a record for the
    // selected month stay selectable and are edited in place (see handleFormSubmit).
    students.forEach(s => {
  ...
  // Fill the score inputs from an existing record (or clear them when none exists).
  // Used so picking a student who already has a score for the month edits it
  // instead of silently overwriting it with blanks.
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // Upsert: when not explicitly editing, reuse the student's existing record for
    // this month so re-entering scores updates it instead of creating a duplicate.
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    // When adding, reflect the chosen student's existing scores for the newly selected month
