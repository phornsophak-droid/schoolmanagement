...
export default function MultiplicationGame({ onBack }: MultiplicationGameProps) {
  ...
  const generateProblem = () => {
    ...
    if (isMultiply) {
      setOperator('×');
      setNum1(Math.floor(Math.random() * 10) + 1); // 1 to 10
      setNum2(Math.floor(Math.random() * 10) + 1); // 1 to 10
    } else {
      setOperator('÷');
      const n2 = Math.floor(Math.random() * 9) + 2; // 2 to 10
      const n1 = n2 * (Math.floor(Math.random() * 10) + 1); // multiple of n2
      setNum1(n1);
      setNum2(n2);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const correct = operator === '×' ? num1 * num2 : Math.floor(num1 / num2);
        ...
        <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-slate-200 max-w-md w-full flex flex-col items-center">
          ...
          {!isPlaying && !gameOver ? (
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-slate-800 mb-2">មេគុណ និងចែកលេខ</h2>
              <p className="text-slate-500 mb-8">អ្នកមានពេល ៦០ វិនាទី ដើម្បីគិតលេខឱ្យបានច្រើនបំផុត។ តើអ្នកអាចធ្វើបានប៉ុន្មាន?</p>
              
              <button
                onClick={startGame}
                className="w-full py-4 px-6 rounded-xl font-bold text-white bg-purple-500 hover:bg-purple-600 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-purple-500/30 text-lg"
              >
            ...
            <div className="w-full">
              <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-100">
                <div className="bg-purple-50 px-4 py-2 rounded-xl flex flex-col items-center">
                  <span className="text-xs text-purple-600 font-bold mb-1 uppercase tracking-wider">ពិន្ទុ</span>
              ...
              <div className="text-6xl font-black text-slate-800 flex items-center justify-center gap-4 mb-8">
                <span>{num1}</span>
                <span className="text-4xl text-purple-500">{operator}</span>
                <span>{num2}</span>
              ...
              <form onSubmit={handleSubmit} className="flex gap-3">
                <input
                  ...
                  className="flex-1 text-center text-3xl font-bold p-4 rounded-xl border-2 border-slate-200 focus:border-purple-500 focus:outline-none focus:ring-4 focus:ring-purple-500/20 transition-all"
                  placeholder="?"
                  autoFocus
                />
                <button
                  type="submit"
                  className="px-6 rounded-xl font-bold text-white bg-purple-500 hover:bg-purple-600 transition-colors shadow-lg shadow-purple-500/30"
                >
                  ឆ្លើយ
                </button>
              </form>
