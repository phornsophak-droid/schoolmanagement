...
export default function SemesterReportCard({ student, students, period, onClose }: SemesterReportCardProps) {
  ...
  const handleDownloadPdf = async () => {
    ...
    if (!el) return;
    setPdfBusy(true);
    try { await exportElementToPdf(el, `ព្រឹត្តបត្រ${periodTitle}_${student.name.replace(/\s+/g, '_')}`, 768); }
  ...
  const handleDownloadImage = async () => {
    ...
    if (!el) return;
    setImgBusy(true);
    try { await exportElementToImage(el, `ព្រឹត្តបត្រ${periodTitle}_${student.name.replace(/\s+/g, '_')}`, 768); }
        ...
        <div id="semester-report-card" className="bg-white rounded-b-2xl shadow-xl p-8 text-slate-800 text-[12px] leading-relaxed">
          ...
          <table className="w-full border-collapse text-[12px]">
            ...
            {/* Single header row; "និទ្ទេស" spans its two value columns (word + letter)
                like the official template. One colSpan, no rowSpan, + the fonts.ready
                wait in exportPdf, so the PDF header no longer drops text. */}
            <thead>
