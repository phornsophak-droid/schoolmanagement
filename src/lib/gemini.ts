...
export async function generateSchoolSummaryAI(dataDigest: string, month: string): Promise<string> {
  const ai = getClient();
  if (!ai) throw new Error('Gemini API key not configured');

  const prompt = `អ្នកគឺជាជំនួយការអប់រំ។ ខាងក្រោមនេះជាទិន្នន័យលទ្ធផលសិក្សារួមសាលា ប្រចាំខែ${month} (ជា JSON)។
...
៣) ចំណុច​គួរ​កែលម្អ​សម្រាប់​ខែ​បន្ទាប់ (ជា​បញ្ជី​ៗ ៣-៥ ចំណុច ដែល​អនុវត្ត​បាន)
